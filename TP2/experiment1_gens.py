            ...
            for(index_param,line_param) in enumerate(line_alg):
                for(index_err,line_err) in enumerate(line_param):
                    for (index_var,gen_values) in enumerate(line_err):
                        ...
                        for gens in gen_values:
                            current_gen_sum+=gens
                        f.write("{0},{1},{2},{3},{4},{5},{6}\n".format(complex_algorithm_names[index_alg], complex_algorithm_param_names[index_alg], complex_algorithm_params_values[index_alg][index_param],variability[index_var],i+1,errors[index_err], current_gen_sum // total_runs))
